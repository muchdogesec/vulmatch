...
class VulmatchDBHelper(DCHelper):
    ...
    def execute_query(
        ...
        cursor = self.db.aql.execute(
            query, bind_vars=bind_vars, count=paginate, full_count=paginate, **aql_options,
        )
...
RETURN KEEP(doc, KEYS(doc, true))
    ...
    def capec_attack_to_vulnerability(self, capec_ids, attack_ids):
        ...
        """
        matched_ids = self.execute_query(query, bind_vars=dict(ext_ids=list(attack_ids + capec_ids)), aql_options=dict(cache=True), paginate=False)
        capec_matches = set()
        attack_matches = set()
    ...
    def cves_to_softwares(self, vulnerable_cve_ids, not_vulnerable_cve_ids):
        ...
        """
        cve_xcpes = self.execute_query(grouping_query, bind_vars=dict(cpe_names=list(vulnerable_cve_ids+not_vulnerable_cve_ids)), aql_options=dict(cache=True), paginate=False)
        ...
        for cve_id, vulerable_criteria_ids, not_vulerable_criteria_ids in cve_xcpes:
            if cve_id in vulnerable_cve_ids:
                vulnerable_groupings.extend(map(generate_grouping_id, vulerable_criteria_ids))
    ...
    def get_groupings_for_indicator(self, indicator, include_x_cpes_vulnerable, include_x_cpes_not_vulnerable):
        ...
        RETURN doc.object_refs
        """,
        bind_vars=dict(grouping_ids=tuple(grouping_ids)), aql_options=dict(cache=True), paginate=False
        )
...
"""
    ...
    def get_cpe_bundle(self, cpe_id: str):
        ...
        indicator_ids = self.execute_query(
            ...
            RETURN d
            """.replace('#filters', '\n'.join(filters)), bind_vars={'matched_ids': list(groupings)}, paginate=False
        )
