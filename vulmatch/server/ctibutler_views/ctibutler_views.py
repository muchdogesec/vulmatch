...
REVOKED_AND_DEPRECATED_PARAMS = [
    OpenApiParameter(
        ...
        description="By default all objects with `revoked` are ignored. Set this to `true` to include them.",
    ),
    OpenApiParameter(
        "include_deprecated",
        type=OpenApiTypes.BOOL,
        description="By default all objects with `x_mitre_deprecated` are ignored. Set this to `true` to include them.",
    ),
]
...
@extend_schema_view(
    ...
    retrieve_object_relationships=extend_schema(
        responses={
            200: AttachedDBHelper.get_paginated_response_schema(
    ...
    retrieve_object_bundle=extend_schema(
        responses={
            200: AttachedDBHelper.get_paginated_response_schema("objects"),
            400: DEFAULT_400_ERROR,
        },
...
class AttackView(viewsets.ViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            description="The STIX ID (e.g. `attack-pattern--0042a9f5-f053-4769-b3ef-9ad018dfa298`, `malware--04227b24-7817-4de1-9050-b7b1b57f5866`)",
        ),
        OpenApiParameter(
            "attack_id",
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The ATT&CK ID, e.g `T1659`, `TA0043`, `S0066`",
        ),
    ]
    ...
    class filterset_class(FilterSet):
        id = BaseCSVFilter(
            help_text="Filter the results using the STIX ID of an object. e.g. `attack-pattern--0042a9f5-f053-4769-b3ef-9ad018dfa298`, `malware--04227b24-7817-4de1-9050-b7b1b57f5866`."
        )
        attack_id = BaseCSVFilter(
            help_text="The ATT&CK IDs of the object wanted. e.g. `T1659`, `TA0043`, `S0066`."
        )
        description = CharFilter(
            help_text="Filter the results by the `description` property of the object. Search is a wildcard, so `exploit` will return all descriptions that contain the string `exploit`."
        )
        name = CharFilter(
    ...
    @decorators.action(
        methods=["GET"], url_path="objects/<str:attack_id>", detail=False
    )
    def retrieve_objects(self, request, *args, attack_id=None, **kwargs):
        return AttachedDBHelper(
            f"nvd_cve_vertex_collection", request
        ).get_object_by_external_id(attack_id, "cve-attack", revokable=True)

    @decorators.action(
        methods=["GET"], url_path="objects/<str:attack_id>/bundle", detail=False
    )
    def retrieve_object_bundle(self, request, *args, attack_id=None, **kwargs):
        return AttachedDBHelper(
            f"nvd_cve_vertex_collection", request
        ).get_object_by_external_id(
            attack_id, "cve-attack", bundle=True, revokable=True
        )
    ...
    def statistics(self, request, *args, **kwargs):
        return response.Response(
            statistics.StatisticsHelper().get_attack_cwe_stats("attack")
        )
...
class CweView(viewsets.ViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The STIX ID (e.g. `weakness--f3496f30-5625-5b6d-8297-ddc074fb26c2`, `grouping--000ee024-ad9c-5557-8d49-2573a8e788d2`)",
        ),
        OpenApiParameter(
            "cwe_id",
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The CWE ID, e.g `CWE-242`, `CWE-250`",
        ),
    ]
    ...
    class filterset_class(FilterSet):
        id = BaseCSVFilter(
            help_text="Filter the results using the STIX ID of an object. e.g. `weakness--f3496f30-5625-5b6d-8297-ddc074fb26c2`, `grouping--000ee024-ad9c-5557-8d49-2573a8e788d2`."
        )
        cwe_id = BaseCSVFilter(
            help_text="Filter the results by the CWE ID of the object. e.g. `CWE-242` `CWE-250`."
        )
        description = CharFilter(
            help_text="Filter the results by the `description` property of the object. Search is a wildcard, so `exploit` will return all descriptions that contain the string `exploit`."
        )
    ...
    def list_objects(self, request, *args, **kwargs):
        ...
        ).get_weakness_or_capec_objects("cve-cwe")
    ...
    def retrieve_objects(self, request, *args, cwe_id=None, **kwargs):
        return AttachedDBHelper(
            "nvd_cve_vertex_collection", request
        ).get_object_by_external_id(cwe_id, "cve-cwe")

    @decorators.action(
        methods=["GET"], url_path="objects/<str:cwe_id>/bundle", detail=False
    )
    def retrieve_object_bundle(self, request, *args, cwe_id=None, **kwargs):
        return AttachedDBHelper(
            "nvd_cve_vertex_collection", request
        ).get_object_by_external_id(cwe_id, "cve-cwe", bundle=True)
    ...
    def statistics(self, request, *args, **kwargs):
        return response.Response(
            statistics.StatisticsHelper().get_attack_cwe_stats("cwe")
        )
...
class CapecView(viewsets.ViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The STIX ID (e.g. `attack-pattern--00268a75-3243-477d-9166-8c78fddf6df6`, `course-of-action--0002fa37-9334-41e2-971a-cc8cab6c00c4`)",
        ),
        OpenApiParameter(
            "capec_id",
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The CAPEC ID, e.g `CAPEC-112`, `CAPEC-699`",
        ),
    ]
    ...
    class filterset_class(FilterSet):
        id = BaseCSVFilter(
            help_text="Filter the results using the STIX ID of an object. e.g. `attack-pattern--00268a75-3243-477d-9166-8c78fddf6df6`, `course-of-action--0002fa37-9334-41e2-971a-cc8cab6c00c4`."
        )
        capec_id = BaseCSVFilter(
            help_text="Filter the results by the CAPEC ID of the object. e.g. `CAPEC-112`, `CAPEC-699`."
        )
        description = CharFilter(
            help_text="Filter the results by the `description` property of the object. Search is a wildcard, so `exploit` will return all descriptions that contain the string `exploit`."
        )
    ...
    def retrieve_objects(self, request, *args, capec_id=None, **kwargs):
        return AttachedDBHelper(
            "nvd_cve_vertex_collection", request
        ).get_object_by_external_id(capec_id, "cve-capec")

    @decorators.action(
        methods=["GET"], url_path="objects/<str:capec_id>/bundle", detail=False
    )
    ...
    @decorators.action(
        methods=["GET"],
        detail=False,
        serializer_class=statistics.CapecStatSerializer(many=True),
        pagination_class=None,
    )
    def statistics(self, request, *args, **kwargs):
        return response.Response(
            statistics.StatisticsHelper().get_attack_cwe_stats("capec")
        )
