...
@extend_schema_view(
    ...
    bundle=extend_schema(
        ...
        + [
            OpenApiParameter(
                "object_type",
                description="The type of STIX object to be returned",
                enum=CVE_BUNDLE_TYPES,
                many=True,
                explode=False,
            ),
            OpenApiParameter(
            ...
            OpenApiParameter(
                ...
                description="Filter vulnerability by `modified`",
            ),
        ],
    ),
...
class CveView(viewsets.ViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            description="The STIX ID, e.g `vulnerability--4d2cad44-0a5a-5890-925c-29d535c3f49e`.",
        ),
        OpenApiParameter(
            "cve_id",
            type=OpenApiTypes.STR,
    ...
    class filterset_class(FilterSet):
        ...
        sort = ChoiceFilter(
            choices=[(v, v) for v in CVE_SORT_FIELDS],
            help_text=textwrap.dedent(
                """
            Sort results by
            """
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(
                "cve_version",
                type=OpenApiTypes.DATETIME,
                description="Return only objects where `modified` value matches query. In format `YYYY-MM-DDThh:mm:ss.sssZ`",
            )
        ]
    )
    ...
    @decorators.action(
        detail=False,
        url_path="objects/<str:cve_id>/versions",
        methods=["GET"],
...
@extend_schema_view(
    list_objects=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "sort", enum=EPSS_SORT_FIELDS, description="Sort results by"
            ),
            OpenApiParameter(
                "epss_min_score",
...
@extend_schema_view(
    ...
    retrieve_object_bundle=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "types",
                description="The types of STIX object to be returned",
                enum=[
                    "software",
                    "relationship",
                    "indicator",
...
class CpeView(viewsets.ViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The full STIX `id` of the object. e.g. `software--93ff5b30-0322-50e8-90c1-1c3f151c8adc`",
        ),
        OpenApiParameter(
            "cpe_name",
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The full CPE name. e.g. `cpe:2.3:a:slicewp:affiliate_program_suite:1.0.13:*:*:*:*:wordpress:*:*`",
        ),
    ]
    ...
    class filterset_class(FilterSet):
        ...
        in_cve_vulnerable = BaseCSVFilter(
            help_text=textwrap.dedent(
                """
        ...
        in_cve_not_vulnerable = BaseCSVFilter(
            help_text=textwrap.dedent(
                """
        ...
        ### more filters
        version = CharFilter(
            help_text="Vendor-specific alphanumeric strings characterising the particular release version of the product (this is the 5th value in the CPE URI)."
        )
        update = CharFilter(
            help_text="Vendor-specific alphanumeric strings characterising the particular update, service pack, or point release of the product (this is the 6th value in the CPE URI)."
        )
        edition = CharFilter(
            help_text="Assigned the logical value ANY (*) except where required for backward compatibility with version 2.2 of the CPE specification (this is the 7th value in the CPE URI)."
        )
        language = CharFilter(
            help_text="Valid language tags as defined by RFC5646 (this is the 8th value in the CPE URI)."
        )
        sw_edition = CharFilter(
            help_text="Characterises how the product is tailored to a particular market or class of end users (this is the 9th value in the CPE URI)."
        )
        target_sw = CharFilter(
            help_text="Characterises the software computing environment within which the product operates (this is the 10th value in the CPE URI)."
        )
        target_hw = CharFilter(
            help_text="Characterises the instruction set architecture (e.g., x86) on which the product being described or identified operates (this is the 11th value in the CPE URI)."
        )
        other = CharFilter(
            help_text="Capture any other general descriptive or identifying information which is vendor- or product-specific and which does not logically fit in any other attribute value (this is the 12th value in the CPE URI)."
        )
    ...
    def retrieve_object_bundle(self, request, *args, cpe_name=None, **kwargs):
        return VulmatchDBHelper(f"nvd_cve_vertex_collection", request).get_cpe_bundle(
            cpe_name
        )
...
class JobView(viewsets.ModelViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            location=OpenApiParameter.PATH,
            description="The Job `id`. You can find Jobs and their `id`s using the Get Jobs endpoint.",
        )
    ]
...
class VendorView(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    class filterset_class(FilterSet):
        q = CharFilter(
            ...
            help_text="Wildcard search on `vendor` name",
        )
...
class ProductView(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    class filterset_class(FilterSet):
        vendor = CharFilter(
            help_text="Filter by `vendor`, must be exact match. `microsof` will not match `microsoft`. Use the GET Vendor endpoint to get a full list of Vendors."
        )
        q = CharFilter(
            field_name="product",
            method="semantic_search",
            help_text="Searches on both `vendor` and `product` fields as a wildcard. `micros code` will match `product=code vendor=microsoft`. Useful if you don't know the exact `vendor` name.",
        )
...
@extend_schema_view(
    retrieve=extend_schema(
        summary="Get CPE Match grouping by Criteria ID",
        description="Get CPE Match grouping by Criteria ID",
        filters=False,
        parameters=[
            OpenApiParameter(
                "version",
                type=OpenApiTypes.DATETIME,
                description="Filter matching criteria by `modified`",
            )
        ],
    ),
    bundle=extend_schema(
        summary="Get bundle from criteria id",
        description="Get bundle from criteria id",
        filters=False,
        parameters=[
            OpenApiParameter(
                "version",
...
class CpeMatchView(viewsets.ViewSet):
    ...
    openapi_path_params = [
        OpenApiParameter(
            ...
            type=OpenApiTypes.STR,
            location=OpenApiParameter.PATH,
            description="The CPE Match Criteria ID, e.g `E7CB4AB4-E082-4C02-BAED-9EA1497C197A`",
        ),
    ]

    class filterset_class(FilterSet):
        criteria_id = BaseCSVFilter(
            help_text=textwrap.dedent(
                """
            Filter the results using a Criteria ID. e.g. `E7CB4AB4-E082-4C02-BAED-9EA1497C197A`
            """
            )
        )
        name = CharFilter(
    ...
    def list(self, request, *args, **kwargs):
        return VulmatchDBHelper("", request).list_groupings()

    def retrieve(self, request, *args, criteria_id=None, **kwargs):
    ...
    def bundle(self, request, *args, criteria_id=None, **kwargs):
        return VulmatchDBHelper(
            f"nvd_cve_vertex_collection", request
        ).get_grouping_bundle(criteria_id)
...
class HealthCheck(viewsets.ViewSet):
    ...
    def service(self, request, *args, **kwargs):
        return Response(
            status=200,
            data=dict(
                ctibutler=self.check_ctibutler(), vulncheck=self.check_vulncheck()
            ),
        )
